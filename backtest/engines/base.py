...
# ─── Dataclasses (ported from backtest/models.py) ───
...
def _run_card_data_sources(config: Dict[str, Any]) -> List[str]:
...
class BaseEngine(ABC):
    ...
    def run_backtest(
        ...
        Args:
            ...
            data_map: code -> OHLCV DataFrame (passed directly by caller).
            signal_map: code -> signal Series (passed directly by caller).
        ...
        # 6. Validation (optional — triggered by config["validation"])
        if config.get("validation"):
            from backtest.engines.validation import run_validation
        ...
        # 8. Trust Layer run card
        from backtest.engines.run_card import write_run_card
        write_run_card(
            run_dir,
            config,
            m,
            data_sources=_run_card_data_sources(config),
